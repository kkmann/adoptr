%\VignetteIndexEntry{Detailed overview of adoptr}
...
\author{
Kevin Kunzmann\\Cambridge University \And Maximilian Pilz\\University of Heidelberg \And Carolin Herrmann\\Charité and BIH \AND Geraldine Rauch\\Charité and BIH \And Meinhard Kieser\\University of Heidelberg
}
\title{The \pkg{adoptr} Package: Adaptive Optimal Designs for Clinical Trials
in \proglang{R}}

\Plainauthor{Kevin Kunzmann, Maximilian Pilz, Carolin Herrmann, Geraldine Rauch, Meinhard Kieser}
\Plaintitle{The adoptr Package: Adaptive Optimal Designs for Clinical Trials in R}
\Shorttitle{\pkg{adoptr}: Adaptive Optimal Clincial Trial Designs in \proglang{R}}
...
straightforward. The package \pkg{adoptr} fills this gap for the common
case of two-stage one- or two-arm trials with (approximately) normally
distributed outcomes. In contrast to previous approaches, \pkg{adoptr}
...
\pkg{adoptr} supports a flexible way of specifying both (composite)
...
\Keywords{optimal design, clinical trials, \proglang{R}}
\Plainkeywords{optimal design, clinical trials, R}


\Address{
  Kevin Kunzmann\\
  Cambridge University\\
  MRC Biostatistics Unit, University of Cambridge Cambridge Institute of
  Public Health, Forvie Site, Robinson Way, Cambridge Biomedical Campus,
  Cambridge CB2 0SR United Kingdom\\
  E-mail: \email{kevin.kunzmann@mrc-bsu.cam.ac.uk}\\
  ...
  E-mail: \email{pilz@imbi.uni-heidelberg.de}\\
  ...
  of Health, Institute of Biometry and Clinical Epidemiology Charitéplatz
  1 10117 Berlin Germany and Berlin Institute of Health (BIH)
  Anna-Louisa-Karsch-Straße 2 10178 Berlin Germany\\
  E-mail: \email{carolin.herrmann@charite.de}\\
  ...
  1 10117 Berlin Germany and Berlin Institute of Health (BIH)
  Anna-Louisa-Karsch-Straße 2 10178 Berlin Germany\\
  E-mail: \email{geraldine.rauch@charite.de}\\

  Meinhard Kieser\\
  University of Heidelberg\\
  Institute of Medical Biometry and Informatics, University of Heidelberg,
  Im Neuenheimer Feld 130.3, 69120 Heidelberg, Germany\\
  E-mail: \email{meinhard.kieser@imbi.uni-heidelberg.de}\\
...
\tikzset{ basic/.style  = {draw, font=\fontsize{10}{10}\sffamily, rectangle}, root/.style   = {basic, rounded corners=2pt, align=center}, level 2/.style = {basic, rounded corners=6pt, align=center, text width=.235\linewidth}, level 3/.style = {basic,  align=left, text width=.215\linewidth}, level 4/.style = {basic,  align=left, text width=.1\linewidth} }
...
<<maximal-sample-size, echo=FALSE, include=FALSE ,cache=FALSE>>=
...
A key quality criterion put forward by the relevant agencies \citep{FDA, EMA}
...
without compromising type one error rate control \citep{Bauer2015} but the
criteria for deciding \emph{which} adaptation should be performed during an
interim analysis and \emph{when} to perform the interim analysis are still
widely based on heuristics.
Bauer \emph{et al.} mention this issue of guiding adaptive decisions at interim
...
Following \citet{MP2011}, \citet{JT2015} developed a principle
...
years \citep{Englert2013, Kunzmann2016, Kunzmann2019} to a setting with
...
specified (cf. Section \ref{sec:Examples}).
...
situation.
The \proglang{R}-package \citep{R} \pkg{adoptr} aims at providing a simple
...
The goal of \pkg{adoptr} is to enable relatively easy experimentation with
different notions of optimality to shift the focus from \emph{how} to optimize
to \emph{what} to optimize.

In the following, we first give a definition of the problem setting
addressed in \pkg{adoptr} and the technicalities of translating the underlying
variational problem to a simple multivariate optimization problem
before motivating the need for an \proglang{R}-package.
We then present the core functionality of \pkg{adoptr} before
addressing the issue of facilitating validation of open-source software in a
regulated environment and discussing potential future work on \pkg{adoptr}.
...
\citep{OBF, Pocock} and for defining the stage-two rejection boundary function
$c_2(\cdot)$ \citep{BK1994, HO1985}.
Often, the inverse-normal combination test \citep{LW1999} is applied and
$c_2(\cdot)$ is defined as a linear function of the stage-one test-statistic
\begin{align} \label{inverse-normal}
...
indeed realized \citep{Zaykin2011}.
...
(cf. \cite{Wassmer2016}, chapter 6.2.5).
...
hypothesis (see, e.g., \cite{JT2015}).
...
\textit{Vice versa}, power can be seen as an unconditional score that is obtained by
...
We demonstrate the consequences of this heuristic in Section \ref{sec:CP_constraint}.
Once the scoring criterion is selected, the design parameters may be chosen
in order to optimize this objective.
The first ones to address this problem were \cite{JT2015}
...
Instead, \cite{JT2015} used a combination test approach to derive $c_2$ given
$n_2(\cdot)$ and $n_1$ (cf. equation (\ref{inverse-normal})).
In \citet{variational}, the authors demonstrated that this restriction is not
...
\section{Direct variational perspective}

In \pkg{adoptr}, a simpler solution strategy than solving the Euler-Lagrange
...
I.e., we adopt a \emph{direct} approach to solving the variational problem.
...
We use cubic Hermite splines \citep{Spline} which are sufficiently flexible,
even for a moderate number of pivots, to approximate any realistic stage-two
sample size and critical value functions.
Since the optimal functions are generally very smooth \citep{variational} they are well suited
to spline interpolation.
Within the \pkg{adoptr} validation report (cf. Section \ref{sec:validation})
...
Since \pkg{adoptr} enables generic objectives (cf. Section \ref{sec:utility_maximization}),
it uses the gradient-free optimizer COBYLA \citep{COBYLA} internally via
the \proglang{R}-package \pkg{nloptr} \citep{nlopt, nloptr}.
...
in a similar way as power (cf. equation (\ref{power})) and
expected sample size (cf. equation (\ref{ess})).
Any such 'integral score' can be computed quickly and reliably in \pkg{adoptr}
via the choice of pivots outlined above.
The correctness of numerically integrated scores is checked in the \pkg{adoptr}
...
The extensive validation suite (cf. Section \ref{sec:validation}) evaluates
...
\section{The need for an R package}

\citet{Bauer2015} state that adequate statistical software for adaptive designs
'is increasingly needed to evaluate the adaptations and to find reasonable
strategies'.

Commercial software such as JMP\textsuperscript{\textregistered} \citep{jmp2020}
or Minitab\textsuperscript{\textregistered} \citep{minitab2020} allow planning
...
\proglang{SAS}\textsuperscript{\textregistered} procedure \code{seqdesign} \citep{sas2020},
PASS\textsuperscript{\textregistered} \citep{pass2019}, or
ADDPLAN\textsuperscript{\textregistered} \citep{addplan2020}.
East\textsuperscript{\textregistered} \citep{east2020} also supports design, simulation and
...
Furthermore, there are various open-source \proglang{R}-packages for the
analysis of multi-stage designs.
The package \pkg{adaptTest} \citep{adaptTest} implements combination tests for
adaptive two-stage designs.
\pkg{AGSDest} \citep{AGSDest} allows estimation and computation of
...
be found in \citet{Bauer2015}, chapter 6, or in \citet{Tymofyeyev2014}.
The choice of software for optimally designing two- or multi-stage designs,
however, is much more limited.
Current \proglang{R}-packages concerned with optimal clinical trial designs
are \pkg{OptGS} \citep{OptGS, OptGS_pub} and \pkg{rpact} \citep{rpact}.
...
The \proglang{R}-package \pkg{adoptr} aims at providing a simple and
...
In particular, \pkg{adoptr} allows to model \textit{a priori} uncertainty
over $\theta$ via prior distributions and thus supports optimization under
uncertainty (cf. Section \ref{sec:uncertainty}).
\pkg{adoptr} also supports the combination of conditional (on $X_1$)
...
(cf. Section \ref{sec:utility_maximization}).
To facilitate the adoption of these advanced trial designs in the clinical trials
community, \pkg{adoptr} also features an extensive test and validation suite
(cf. Section \ref{sec:validation}).

In the following, we outline the key design principles for \pkg{adoptr}.

\begin{enumerate}
\item \textbf{Interactivity:}
A major advantage of the \proglang{R}-programming language is its
...
validation report (cf. Section \ref{sec:validation}).
...
the object-oriented approach of \pkg{adoptr} facilitates the definition of custom
scores and constraints that seamlessly integrate with the remainder of the package.
\end{enumerate}

\section{Adoptr's structure}
\label{sec:structure}

The package \pkg{adoptr} is based on \proglang{R}'s S4 class system.
This allows to use multiple dispatch on the classes of multiple arguments
to a method.
In this section, the central components of \pkg{adoptr} are described briefly.
Figure \ref{fig:class-diagram} gives a structural overview of the main
classes in \pkg{adoptr}.
...
\begin{tikzpicture}[level 1/.style={sibling distance=4cm},edge from parent/.style={-,draw}]
\node[root] {\textbf{Score}  \\ \textit{evaluate} \\ \textit{composite}}
  child {node[level 2, yshift = -5pt] (c1) {\textbf{UnconditionalScore}}}
  child {node[level 2, yshift = -10pt] (c2) {\textbf{ConditionalScore} \\ \textit{expected}}};
\begin{scope}[every node/.style={level 3}]
\node [below of = c1, yshift=-5pt] (c11) {Power};
\node [below of = c11, yshift = 10pt] (c12) {ExpectedSampleSize};
\node [below of = c12, yshift = 10pt] (c13) {N1};
\node [below of = c13, yshift = 10pt] (c14) {AverageN2};
\node [below of = c2, xshift=0pt] (c21) {ConditionalPower};
\node [below of = c21, yshift = 10pt] (c22) {ConditionalSampleSize};
\end{scope}
...
\begin{tikzpicture}[baseline=-50*\the\dimexpr\fontdimen22\textfont2\relax]
\node[root] (d1) {\textbf{TwoStageDesign} \\ \textit{make\_fixed} \\ \textit{make\_tunable}};
\begin{scope}[every node/.style={level 3}]
\node [below of = d1, yshift=-20pt] (d11) {GroupSequentialDesign};
\node [below of = d11, yshift = 10pt] (d12) {OneStageDesign};
\end{scope}
\draw[-] (d1.west) |- (-2,0);
\foreach \value in {1,2}
  \draw[-] (-2, 0) |- (d1\value.west);
\end{tikzpicture}

\\

\begin{tikzpicture}[level 1/.style={sibling distance=40mm},edge from parent/.style={->,draw}]
\node[root] (e1) {\textbf{DataDistribution}};
\begin{scope}[every node/.style={level 4}]
\node [below of = e1, yshift=0pt] (e11) {Normal};
\node [below of = e11, yshift = 10pt] (e12) {Binomial};
\end{scope}
\foreach \value in {1,2}
  \draw[-] (e1.191) |- (e1\value.west);
\end{tikzpicture}

&

\begin{tikzpicture}[level 1/.style={sibling distance=40mm}, edge from parent/.style={->,draw}]
\node[root] (f1) {\textbf{Prior} \\ \textit{condition}};
\begin{scope}[every node/.style={level 3}]
 \node [below of = f1, yshift=0pt] (f11) {ContinuousPrior};
 \node [below of = f11, yshift = 10pt] (f12) {PointMassPrior};
\end{scope}
\draw[-] (f1.west) |- (-2, 0);
\foreach \value in {1,2}
  \draw[-] (-2,0) |- (f1\value.west);
\end{tikzpicture}

\end{tabular}
\caption{\label{fig:class-diagram} Overview of the most important classes and
methods (in italic) in the \proglang{R}-package \pkg{adoptr}.
A subclassing relationship is indicated by a connecting line to the
corresponding super class above it.
The most important methods for each class are listed under the respective
class name in italic font.}
\end{figure}

To compute optimal designs, an object of class \code{UnconditionalScore}
must be defined as objective criterion.
\pkg{adoptr} distinguishes between \code{ConditionalScore}s and
\code{UnconditionalScore}s (cf. Section \ref{sec:setting}).
All \code{Score}s can be evaluated using the method \code{evaluate}.
For unconditional scores, this method only requires a \code{Score} object and a
\code{TwoStageDesign} object,
for conditional scores (like conditional power), it also requires the interim
outcome $x_1$.
Note that any \code{ConditionalScore} $S(\mathcal{D}|X_1=x_1)$
can be converted to an \code{UnconditionalScore}
$S(\mathcal{D}) = \boldsymbol{E}\big[S(\mathcal{D}|X_1)\big]$ using the method
\code{expected}.
The two most widely used conditional scores are pre-implemented as \code{ConditionalPower} and \code{ConditionalSampleSize}.
Their unconditional counterparts are \code{Power} and
\code{ExpectedSampleSize}.
Further predefined unconditional scores are \code{AverageN2}, evaluating the average
of the stage-two sample size (improper prior) and
\code{N1} measuring the first-stage sample size.
...
Users are free to define their own \code{Score}s
(cf. the vignette 'Defining New Scores' \citep{new-scores}).
Moreover, different \code{Score}s can be composed to a single one by the
function \code{composite} (cf. Section \ref{sec:utility_maximization}).
...
The function \code{minimize} takes an unconditional score as objective and
a set of constraints and optimizes the design parameters.

In \pkg{adoptr}, different kinds of designs are implemented.
The most frequently applied case is a \code{TwoStageDesign}, i.e., a design
with one interim analysis and a sample size function that varies with the
interim test statistic.
Another option is the subclass \code{GroupSequentialDesign} which restricts
the sample size function on the continuation region to a single number, i.e.,
${n_2(x_1) = n_2\ \forall x_1\in[\,c_1^f, c_1^e\,]}$.
Additionally, \pkg{adoptr} supports the computation of optimal
\code{OneStageDesign}s, i.e., designs without an interim analysis.
Technically, one-stage designs are implemented as subclasses of
\code{TwoStageDesign}
since they can be viewed as the limiting case for $n_2\equiv0$ and $c_1^f=c_1^e$.
Hence, all methods that are implemented for \code{TwoStageDesign}s also work
for \code{GroupSequentialDesign}s and \code{OneStageDesign}s.
Users can chose to keep some elements of a design fixed during optimization
using the methods \code{make\_fixed} (cf. Section \ref{sec:make_fixed}).

The joint data distribution in \pkg{adoptr} consists of two elements.
The distribution of the test statistic is specified by an object of class
\code{DataDistribution}.
Currently, the two options \code{Normal} and \code{Binomial} are implemented.
The logical variable \code{two\_armed} allows the differentiation between
one- and two-armed trials.
Furthermore, \pkg{adoptr} supports prior distributions on the effect size.
These can be \code{PointMassPrior}s (cf. Section \ref{sec:standard_case})
as well as \code{ContinuousPrior}s (cf. Section \ref{sec:uncertainty}).

In the following section, more hands on examples demonstrate the capabilities of \pkg{adoptr} and its syntax.
...
\Sexpr{ceiling(pwr::pwr.t.test(d = .3, sig.level = .025, power = .9, alternative = "greater")$n)}
per group.

Using \pkg{adoptr}, the two-stage design minimizing the expected sample size under
...
The \textit{two\_armed} parameter allows to switch between single-armed and
two-armed trials.
...
@
A \code{Power} score requires the data distribution and the prior
to be specified.
For this example, we choose \code{PointMassPrior}s with the entire probability mass
...
@
Since \pkg{adoptr} internally relies on the COBYLA implementation of
\pkg{nloptr}, an initial design is required.
A heuristic initial choice is provided by the function \code{get_initial_design}.
...
@
Alternatively, one might also \code{evaluate} a constraint object directly via
<<collapse=FALSE>>=
...
Calling \code{evaluate} on a constraint object then simply returns the left-hand
side of the inequality.
The actual optimization is started by invoking \code{minimize}
<<case-1-optimization>>=
...
The call to \code{minimize()} is designed to be as close as possible to
the mathematical formulation of the optimization problem
and returns both the optimized design (\code{opt1$design}) as well as the full
\pkg{nloptr} return value
with details on the optimization procedure (\code{opt1$nloptr_return}).

A \code{summary} method for objects of the class \code{TwoStageDesign} is available
to quickly evaluate a set of \code{ConditionalScores} such as conditional power
as well as \code{UnconditionalScores} such as power and expected sample size.
<<>>=
cp <- ConditionalPower(dist = datadist, prior = alternative)
summary(opt1$design, "Power" = power, "ESS" = ess, "CP" = cp)
@
\pkg{adoptr} also implements a default plot method for the overall sample size and
the stage-two critical value as functions of the first-stage test statistic $x_1$.
The plot method also accepts additional \code{ConditionalScores} such as conditional power.
...
\begin{figure}
<<plot1, fig.height=2.25*1.25, fig.width=6*1.25, out.height="2.25in", out.width="6in">>=
plot(opt1$design, `Conditional power` = cp)
@
\caption{\label{fig:standard-case}Optimal sample size, critical value, and conditional power plotted against the interim test statistic (built-in plot method).}
\end{figure}
...
would be linear by definition (cf. equation (\ref{inverse-normal})).
Since the optimal shape of $c_2(\cdot)$ is not linear (but almost), inverse-normal
combination methods are slightly less efficient (cf. \citet{variational} for a more
detailed discussion of this issue).

\subsection{Optimization under uncertainty}
\label{sec:uncertainty}
\pkg{adoptr} is not limited to point priors but also supports arbitrary
...
This score can be defined in \pkg{adoptr} by first conditioning the prior.
<<case-2-pwr-mcr>>=
...
\pkg{adoptr} exposes the \pkg{nloptr} options via the argument \code{opts}.
...
\Sexpr{round(evaluate(ess, opt2$design), 1)}.
This points out the increased uncertainty on $\theta$ since the expected sample
size under the prior of the optimal design from Section \ref{sec:standard_case}
amounts to a lower value of \Sexpr{round(evaluate(ess, opt1$design), 1)}.


\subsection{Utility maximization and composite scores}
\label{sec:utility_maximization}
\pkg{adoptr} also supports composite scores.
This can be used to derive utility maximizing designs by defining an objective
function combining both expected power (cf. equation \ref{expected-power}) and
...
This can be interpreted as a pricing mechanism (cf. \citet{Kunzmann2019}):
...
A composite score can be defined via any valid numerical \proglang{R}
...
\Sexpr{evaluate(mss, opt3$design)} and is distinctly smaller than in the case of a
hard power constraint under a
point prior (maximal sample size: \Sexpr{evaluate(mss, opt1$design)})
or a continuous prior (maximal sample size: \Sexpr{evaluate(mss, opt2$design)}).
\begin{figure}
<<plot2, echo=FALSE, warning=FALSE, fig.height=2.25*1.25, fig.width=6*1.25, out.height="2.25in", out.width="6in">>=
x1    <- seq(0, 3, by = .01)
theta <- seq(0, .4, by = .01)
...
@
\caption{\label{fig:comparison}Comparison of optimal designs under a point prior, a continuous prior, and a utility maximization approach.}
\end{figure}




\subsection{Conditional power constraint}
\label{sec:CP_constraint}
\pkg{adoptr} also allows the incorporation of hard constraints on conditional
...
(\Sexpr{evaluate(ess, opt4$design) %>% round(1)} with conditional power constraint
vs. \Sexpr{evaluate(ess, opt1$design) %>% round(1)} without).
...
\begin{figure}
<<plot3, echo=FALSE, warning=FALSE, fig.height=2.25*1.25, fig.width=6*1.25, out.height="2.25in", out.width="6in">>=
   x1 <- seq(0, 3, by = .01)
...
@
    \caption{\label{fig:cp-constraint}Optimal designs with and without conditional power constraint.}
\end{figure}


\subsection{Keeping design parameters fixed}
\label{sec:make_fixed}
...
Returning to the standard case discussed in Section \ref{sec:standard_case}, assume that a stage-one
per-group sample size of exactly 80 individuals per group is required
($n_1 = 80$) instead of the optimal value of $n_1^* = \Sexpr{round(opt1$design@n1)}$.
...
\pkg{adoptr} supports such considerations by allowing to fix specific values of
a design:
<<>>=
...
(\Sexpr{round(evaluate(ess, opt5$design), 1)} compared to
\Sexpr{round(evaluate(ess, opt1$design), 1)} in the less restricted case).

\begin{figure}
<<plot4, echo=FALSE, warning=FALSE, fig.height=2.25*1.25, fig.width=6*1.25, out.height="2.25in", out.width="6in">>=
...
@
\caption{\label{fig:tunable}Comparison of fully optimal design and optimal design with fixed first-stage sample size.}
\end{figure}
...
validate any software used to work with or produce records \citep{crf11}.
The exact scope of regulations such as CRF 11 is sometimes difficult to assess,
and it is not always clear which regulations apply to \proglang{R}
packages used in a production environment \citep{r-fda}.
Irrespective of the applicability of the CRF 11 to \pkg{adoptr}, the design
...
again following CRF 11 and the remarks in \citet{r-fda},
a 'validated \proglang{R}-package' does not exist
...
\pkg{adoptr} implements the following measures:

\begin{enumerate}
\item \textit{Open-source development:} The entire development of \pkg{adoptr} is
...
\item \textit{CRAN releases:} Regular CRAN \citep{cran} releases of updated versions
maximize visibility and add an additional layer of testing and quality control.
...
\item \textit{Unit testing:} \pkg{adoptr} implements an extensive test suite using
the package \pkg{testthat} \citep{testthat, testthat_pub} which allows spotting
...
\item \textit{Continuous Integration / Continuous Deployment:}
\pkg{adoptr} makes extensive use of the continuous integration and deployment
services of Travis-CI \citep{travis}.
...
\item \textit{Coverage analyses:} To document the extent to which the test suite covers
the package code, \pkg{adoptr} relies on the codecov \citep{codecov}
online service in conjunction with the \pkg{covr} package \citep{covr}.
...
\item \textit{Online documentation:}
Beyond the standard documentation generated using \pkg{roxygen2} \citep{roxygen},
we also make use of the \pkg{pkgdown} \citep{pkgdown}
package and the free GitHub pages service
to publish a static \proglang{html} version of the documentation online at
...
\item \textit{Extended validation report:} There are limits to what can be done in the
...
comparison with existing packages such as \pkg{rpact}, we implemented an external
...
\pkg{bookdown} \citep{bookdown, bookdown_pub} package.
...
Within the report, we still use \pkg{testthat} to conduct formal tests.
...
The main motivation of implementing \pkg{adoptr} in \proglang{R} is the fact
that this is by far the most common programming language used by the target
audience.
Note, however, that using \proglang{R} for generic nonlinear constraint
...
Since one of the design principles of \pkg{adoptr} is extensibility,
the ability to support custom objective functions is central.
In \proglang{R}, this implies that one has to resort to either a
...
An interesting alternative to \proglang{R} and \pkg{nloptr} would therefore be
\proglang{Julia} \citep{Julia} and the \pkg{JuMP} framework
for numerical programming \citep{JuMP}.
This framework allows interfacing generic nonlinear solvers via a common
interface and, leveraging \proglang{Julia}'s excellent automatic-differentiation
...
The first two authors contributed equally to its completion.
