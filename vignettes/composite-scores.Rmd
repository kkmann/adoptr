...
ess   <- SampleSize(datadist, H_1)
power <- Power(datadist, H_1)
toer  <- Power(datadist, H_0)
```

Adoptr supports such `CompositeScores` via the `compose` function:
```{r sum}
objective <- compose({ess - 50*power})
```
