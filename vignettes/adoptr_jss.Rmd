...
knitr::opts_chunk$set(engine='R', highlight=FALSE, prompt=TRUE, tidy=FALSE)
...
\begin{align} 
  c_2(x_1) = \frac{c - w_1 x_1}{w_2} (\#eq:inverse-normal)
\end{align}
...
\begin{align} 
\operatorname{Power_\theta} = \boldsymbol{E}_\theta\big[\operatorname{CP}(X_1)\big]. (\#eq:power)
\end{align}
...
\boldsymbol{E}_{\theta} \big[n_1 + n_2(X_1) \big] (\#eq:ess)
\end{align}
...
Within the **adoptr** validation report (cf. Section [7](#sec:validation))
...
\big(\widetilde{x}_1^{(i)}\big), (\#eq:ess-gaussquad)
\end{align}
...
in a similar way as power (cf. Equation \@ref(eq:power)) and
expected sample size (cf. Equation \@ref(eq:ess)).
...
The extensive validation suite (cf. Section [7](sec:validation)) evaluates 
...
validation report (cf. Section [7](#sec:validation)).
3. **Extensibility:**
...
\begin{align}
\boldsymbol{E} \Big[
\boldsymbol{Pr}_\theta\big[X_2>c_2(X_1)\big] \, \Big| \, \theta \geq 0.1 \Big] (#eq:expected-power)
\end{align}
...
`r round(evaluate(ess, opt2$design), 1)`.
...
amounts to a lower value of `r round(evaluate(ess, opt1$design), 1)`.
...
function combining both expected power (cf. Equation \@ref(#eq:expected-power)) and
...
`r evaluate(mss, opt3$design)` and is distinctly smaller than in the case of a
hard power constraint under a
point prior (maximal sample size: `r evaluate(mss, opt1$design)`)
or a continuous prior (maximal sample size: `r evaluate(mss, opt2$design)`).
...
power (cf. Equation \@ref(#eq:power)).
...
(`r evaluate(ess, opt4$design) %>% round(1)` with conditional power constraint
vs. `r evaluate(ess, opt1$design) %>% round(1)` without).
...
is required ($n_1 = 80$) instead of the optimal value of $n_1^* = `r round(opt1$design@n1)`$.
...
(`r round(evaluate(ess, opt5$design), 1)` compared to
`r round(evaluate(ess, opt1$design), 1)` in the less restricted case).
