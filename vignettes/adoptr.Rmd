...
to stop or modify an ongoing trial, cf. [REFEENCE BAUER et al 2016].
...
decision at the interim analyis.
...
sub-optimal.
Intuitively, this can be seen from the fact that the derivation of optimal
weights for a two-stage test build on the notion of a hypothetical overall
sample size which might never be realized in practice [REFERENCE].
...
$\big(n_1, c_1^e, c_1^f, n_2(\cdot), c_2(\cdot)\big)$ directly for a given
objective function and potentially a set of constraints.
...
Consider the case of a two-stage design for a single-arm trial
aiming to reject the null hypothesis $\mathcal{H}_0: \delta \leq 0$ 
where $\delta$ is the population-mean response. 
...
over the unknown paramter $\delta$.
