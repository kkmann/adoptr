...
During the accrual time, recruit approximately 46 patients (per group; the exact number does not matter as only the number of events is important) and conduct the interim analysis after $2 \cdot 32= 64$ events have been observed overall. Assume, $r$ patients have not had an event until the interim analysis. These patients are censored for the computation of $L_1$. 

After the interim analysis, we have $2\cdot \texttt{n2(L_1)}$ ($\texttt{n_2}$ denotes the **adoptr** function) events that should be observed during the second stage. But of course, $r$ patients are still in the trial, so we only need to recruit $2\cdot \texttt{n2(L_1)}\hat{d}-r$ new people. 

When the required number of events have been observed, we now conduct the final analysis using $Z_2$ instead of $L_{\Delta}$.
