...
Currently, `adoptr` only supports the specification of a single fixed reference value fo $p_C$, while general prior distributions are supported for $\theta$. 
...
Assume we want to plan a two-armed trial with $p_C=0.3$.
...
Assume, we estimate an event rate of $\hat{d}=0.7$. We assume to have a two-armed design.

```{r}
datadist <- Survival(0.7,two_armed = TRUE)
```

As $\theta$ denotes the hazard ratio, we have $\theta=1$ under the null.
Under the alternative, we assume to have $\theta=1.7$ without a prior distribution.
