...
data to a test-statistic which is approximately. In the following, we will
...
$\hat{p}_0=\frac{\frac{1}{n}\sum_i^{n}X_i^T+\frac{1}{n}\sum_i^{n}X_i^C}{2n}$.
...
Let us furthermore postulate a normal prior distribution for $\theta$ with expectation $\mu=0.2$ and standard deviation $\sigma=0.2$, which was truncated to the interval $(-.29,0.69)$. It is necessary to use a truncation to ensure that $p_T \in (0,1)$. Aft
```{r}
...
During the accrual time, recruit approximately $\frac{32}{0.7}\approx 46$ patients (per group; the exact number does not really matter as the number of events is the important value) and conduct the interim analysis after $2 \cdot 32= 64$ events have been observed overall. Assume, $q$ patients have not had an event until the interim analysis. These patients are censored for the computation of $L_1$. 

After the interim analysis, we have $2\cdot \texttt{n2(L_1)}$ ($\texttt{n_2}$ denotes the `adoptr` function) events that should be observed during the second stage. But of course, $q$ patients are still in the trial, so we only need to recruit $\frac{2\cdot \texttt{n2(L_1)}}{\hat{d}}-q$ new people. 
...
#### Das hier mit dem vorherigen Kapitel integrieren


During the accrual time, recruit approximately 46 patients (per group; the exact number does not matter as only the number of events is important) and conduct the interim analysis after $2 \cdot 32= 64$ events have been observed overall. Assume, $q$ patients have not had an event until the interim analysis. These patients are censored for the computation of $L_1$. It is important to know the time span these subjects have been in the trial until the analysis is conducted.
...
$c_f$ and $c_e$, the number of events to observe until the conduct of
...
and the expected rate of events $d$.

Let us say we want to plan a two-armed trial trial, where we postulate
...
Our optimized design should minimize the number of required events.
(Nico, kannst du einen Wrapper namens "ExpectedNumberOfEvents" bauen, der
genau das gleiche macht wie ExpectedSampelSize?)

```{r}
exp_no_events <- ExpectedSampleSize(datadist,H_1)
...
important information thresholds to adhere testing purposes, while $N_1$ and
...
approach than the $N_2 = n_2 / d$ formula.
