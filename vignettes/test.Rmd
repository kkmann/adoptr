...
c1f = .0
c1e = 3.5
knots <- seq(c1f + .01, c1e - .01, length.out = 25)
weights_n2 <- rep(40.0, length(knots))
weights_n2[knots < c1f | knots > c1e] <- 1.0

weights_c2 = rep(1.96, length(knots))
weights_c2[knots < c1f] <- 100
weights_c2[knots > c1e] <- .25
...
plot(seq(-1, 4, .01), n2(d, seq(-1, 4, .01)))
plot(seq(-1, 4, .01), c2(d, seq(-1, 4, .01)))
...
objective <- function(x) {
  d  <- BSDesign_(c(25, c1f, c1e, x), d@knots, 3L)
  #p1 <- (d@knots < d@early_stopping_for_futility) * (d@knots - d@early_stopping_for_futility)^2
  #p2 <- (d@knots > d@early_stopping_for_efficacy) * (d@knots - d@early_stopping_for_efficacy)^2
  penalty <- eval(ess, d) + .01*mean((diff(d@weights_n2))^2)
  return(penalty)
}
objective(tail(get_tunable_parameters(d, simplify = TRUE), -3))
...
constraint(tail(get_tunable_parameters(d, simplify = TRUE), -3))

ub <- c(
  numeric(length(knots)) + 100,
  numeric(length(knots)) + 5
)

lb <- c(
  numeric(length(knots)),
  numeric(length(knots)) - 5
)

res <- nloptr::nloptr(
  tail(get_tunable_parameters(d, simplify = TRUE), -3),
  lb = lb,
  ub = ub,
  eval_f      = objective,
...
d2 <- BSDesign_(c(25, c1f, c1e, res$solution), d@knots, 3L)
d2
plot(d2@knots, d2@weights_n2 + d2@n1)
plot(z1, n2(d2, z1) + 25, 'l')
plot(z1, c2(d2, z1), 'l')
